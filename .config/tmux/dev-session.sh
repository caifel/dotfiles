...
API_DIR="${API_DIR:-$PROJECTS_DIR/ajedrezlapaz-api}"
...
tmux new-session -d -s "$SESSION_NAME" -n web -c "$APP_DIR" "$SHELL_BIN"
...
tmux new-window -t "$SESSION_NAME:" -n run -c "$APP_DIR" "$SHELL_BIN"
